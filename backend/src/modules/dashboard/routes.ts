...
dashboardRouter.get("/stats", requireAuth, permit("dashboard.read"), async (_req, res) => {
  ...
  const [totalArticles, todayArticles, draftArticles, reviewArticles, users, popular] = await Promise.all([
  ...
  res.json({ totalArticles, todayArticles, draftArticles, reviewArticles, users, totalViews: views._sum.viewsCount ?? 0, popular });
});
