...
async function main() {
  await prisma.role.upsert({
    ...
    update: {},
    create: { name: "SUPER_ADMIN", permissions: { create: permissions.map((key) => ({ key })) } }
  });
  await prisma.role.upsert({
    where: { name: "EDITOR" },
    update: {},
    create: {
      name: "EDITOR",
      permissions: { create: permissions.filter((key) => key.startsWith("articles") || key === "dashboard.read").map((key) => ({ key: `editor.${key}` })) }
    }
  });
